...
- [x] `ClaudeCodeProvider::new(cursors: BufferHandle)` implementing `Provider` with `name()` = `"claude_code"`; `run` ticks every `config.claude_code.poll_interval` seconds (`MissedTickBehavior::Delay`)
- [x] per tick, for each configured root: a missing `projects` directory is logged once at info (not every tick) and skipped; otherwise list `<projects>/*/*.jsonl` (exactly two levels - this excludes `subagents/`), sorted by path
- [x] per file: load its cursor (cache cursors in memory after the first read; read from the buffer only on first sight), call `read_increment` repeatedly until it returns `None`, sending each increment as `Emission::awaiting_commit(drafts, Some(cursor))` - also when `drafts` is empty but the offset moved, so a file of only tool lines is not re-read forever - and advancing the in-memory cursor only after `committed.await` succeeds (on failure: log and move on to the next file; the next tick retries from the durable cursor)
- [x] a send failure on `out` (runtime gone) returns `Ok(())`, like `browser_history`; any IO error on one file is logged and the loop continues with the next file - the provider returns `Err` only for a buffer that refuses cursor reads
- [x] register it in `main.rs` under `supervise` with `Backoff::default()`, abort and await it on shutdown with the others, and add `claude_code` to `PROVIDERS`
- [x] write tests with a real temp buffer (`Buffer::open` as in `runtime::tests`) and a temp root: a first run ships every message and session record of the fixture once; a second run over an unchanged file ships nothing; appended lines ship on the next tick; a missing root does not fail the provider; a restarted provider resumes from the committed cursor without re-shipping
